      ...
      steps {
        catchError {
          withCredentials(bindings: [usernamePassword(credentialsId: 'ad99e083-f143-411f-81b1-a87f62c2a72b', usernameVariable: 'FTPUserName', passwordVariable: 'FTPPassword')]) {
            ...
            sh "lftp -e 'mput -r bikeshop-admin-frontend/build/*; bye' -u $FTPUserName,$FTPPassword 192.168.1.10"
            sh "lftp -e 'mput -r bikeshop-user-frontend/build/*; bye' -u $FTPUserName,$FTPPassword 192.168.1.9"
          }
          echo "Deployed!"
        }
      }
